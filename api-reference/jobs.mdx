...
| `language` | string | Yes | none | 2-character language code |
| `depth` | integer | No | `5` | Min `1`, max `20` |
| `include_emails` | boolean | No | `false` | Enables email scraping from business websites |
| `max_images` | integer | No | `0` | Image cap **per place**, max `500` |
| `max_reviews` | integer | No | `0` | Review cap **per place**, max `500` |
...
| `max_images` | integer | No | `0` | Number of images to collect **per place**. 0 means skip images. Range: 0 to 500. |
...
| `include_emails` | boolean | Whether email extraction was included in the estimate. |
